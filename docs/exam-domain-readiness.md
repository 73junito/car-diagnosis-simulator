...
Production deployment uses `npm run cloudflare:exam:deploy`. That command first validates the academic-pathway contract and synchronizes the canonical `data/curriculum/` JSON into the isolated `exam-site` asset tree before Wrangler deploys `wrangler.exam.jsonc`. The separate Worker keeps exam routing isolated from the public and training applications. Certification infrastructure remains deferred under `docs/PAYMENT_ENTITLEMENT_CONTRACT.md`.
