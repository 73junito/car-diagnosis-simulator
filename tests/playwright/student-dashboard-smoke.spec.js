...
test.describe('Student dashboard smoke', ()=>{
  test('loads and basic interactions', async ({ browser })=>{
    ...
    // wait for at least one rendered card (guard against race where event already fired)
    ...
    console.log('SHOWN IDS COUNT', shownIds.length, 'DOM IDs COUNT', domIds.length);
    expect(domIds.length).toBe(shownIds.length);
    // identity parity: ensure DOM IDs match shown IDs
    const sort = arr => arr.slice().sort();
    expect(sort(domIds)).toEqual(sort(shownIds));
